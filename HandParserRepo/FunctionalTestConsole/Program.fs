...
let main argv = 
    ...
    let isVpipHandAction action = match action.Action with
        | HandActionType.Bet 
        | HandActionType.Call
        | HandActionType.Raise -> true
        | _ -> false
    ...
    let calculateFlagStat (actions, checkcondition) = match actions|> Seq.exists(checkcondition) with
        | true -> 1
        | false -> 0

    let calculateDecimalSumStat (action:'a, checkCondition, valSelector:'a->'b) = actions |> Seq.where(checkCondition) |> Seq.map(valSelector) |> Seq.sum
